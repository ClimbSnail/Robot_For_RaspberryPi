...
class LeServo:
	...
	# 读取folderPath目录下所有 以pre表示字符打头的的动作文件所在的路径
	...
	def readAllAtion(self, paths, aNumMark, saveMemNum):
		...
		for path in paths:
			...
			pos = self.findNextMark(aNumMark, datas, 0, datLen)
			if pos == -1:
				continue
			while pos<datLen:
				numPos = pos
				# 查找第pos+2行后一个aNumMark标志所在的下标位置
				endPos = self.findNextMark(aNumMark, datas, pos+1, datLen)
				if endPos == -1:
					endPos = datLen
				actionDatas = []
