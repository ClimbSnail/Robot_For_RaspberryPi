...
import UartRaspberryPi
from GPIO import *		# 导入拓展版的类库
from ReadAction import *		# 导入读动作数据的库
from BaiduSpeak import *
from TuringRobot import *
		...
		if d["awakeUpCnt"] > 6:
			...
			if ret["err_msg"] == "success.":
				...
				#	d["RobotOrder"] = ask_text
					...
					req = TR.talk(ask_text)["text"]  # 获取回答
					print('你: %s' % ask_text)
...
# Image_Source定义图像来源 Camera 与 MJPEG_Stream, windows上建议参数Camera
def get_image(Image_Source, d):
	global Running
	...
	if Image_Source == "Camera":
		...
		capture.set(4, 480)
	while True:
		if Running and Image_Source == "MJPEG_Stream":
			try:
...
if __name__ == "__main__":
	...
	if model == "thread":
		...
		d["Image_Source"] = "Camera"
		d["Voice_EN"] = True	#语音交互使能标志位
		d["awakeUpCnt"] = 0	#语音交互计数值
		d["RobotOrder"] = ""	# 判断机器人控制信号
		
		thread_object = []
		# 语音交互线程
		thread_object.append(threading.Thread(target=Voice, args=(q, d) ))
		# 摄像头线程
		thread_object.append(threading.Thread(target=Face, args=(q, d) ))
		# 播放器线程
		thread_object.append(threading.Thread(target=Player, args=(q, d) ) )
		# 离线语音唤醒线程
		thread_object.append(threading.Thread(target=VoiceAwakeUp, args=(q, d, ) ) )
		# 机器人控制线程
		thread_object.append(threading.Thread(target=Action, args=(q, d) ) )
	...
	if model == "process":
		...
		process_object = []
		# 语音交互进程
		process_object.append(Process(target=Voice, args=(q, d,) ) )
		# 摄像头进程
		process_object.append(Process(target=Face, args=(q, d,) ) )
		# 播放器进程
		process_object.append(Process(target=Player, args=(q, d,) ) )
		# 离线语音唤醒进程
		process_object.append(Process(target=VoiceAwakeUp, args=(q, d, ) ) )
		# 机器人控制进程
		process_object.append(Process(target=Action, args=(q, d) ) )
